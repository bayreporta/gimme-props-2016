...
var propSelected = null, propID, tutorial = false, lockSlide = false, propMode = false, returning, firstPropPass;
...
function configureSlides(direct){
	...
	//RETURNING TO PROP
		...
		curSlide = '#proplist'; downSlide = '#prop59';returning = false;
		setTimeout(function(){
			$('.' + propSelected).add('.propheader').remove();
		}, 300);
		$('.downcaret').animate({opacity:.3}, 300); $('.propcaretleft').add('.propcaretright').fadeOut(300);
	}
	//ABOUT
	...
	//PROP LIST 51
	else if (curSlide == '#proplist' && direct == 'down' || curSlide == '#prop59' && direct == 'up'){   
		clearAnimations(); dropHammer(true);
		curSlide = '#prop51'; upSlide = '#proplist'; downSlide = '#prop59';rightSlide = '#tutorial'; propSelected = 'prop51';tutorial = true;
		$('.downcaret').add('.upcaret').add('.rightcaret').animate({opacity:.3}, 300); $('.leftcaret').animate({opacity:0}, 300);
	}	
	//PROP LIST 59
	else if (curSlide == '#prop51' && direct == 'down'){   
		rainingCoins();
		curSlide = '#prop59'; upSlide = '#prop51'; rightSlide = '#tutorial'; propSelected = 'prop59';tutorial = true;
		$('.downcaret').add('.upcaret').add('.rightcaret').animate({opacity:.3}, 300); $('.leftcaret').animate({opacity:0}, 300);
	}	
